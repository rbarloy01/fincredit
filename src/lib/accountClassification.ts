...
export function classifyAccount(statementType: string, name: string, sectionPath?: string | null): string {
  ...
  const isPasivoName = /(pasivo|proveedor|acreedor|deuda|obligacion|prestamo|impuesto|seguro|social|imss|isr|iva|ptu|provision|cuentas?porpagar|cxp)/.test(n);
  ...
  if (/(activo|caja|banco|efectivo|disponibilidad|cliente|cuentas?porcobrar|inventario|propiedad|equipo|intangible)/.test(n)) return 'ACTIVO';
